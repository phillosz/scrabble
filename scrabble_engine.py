...
def sack_append_letters_remove(player, letter, sack_placeholder):
    ...
    sack_refill(player, sack_placeholder)
def score_counter(player, letter_scores, arr):
    score = 0
    ...
    if score_multiplier_func_3W(player["word"], player["x"], player["y"], player["direction"], arr) != False:
        ...
        if len(player["joker_letter"]) != 0:
            score -= 3*sum(player["joker_value"])
            player["joker_value"] = []
    if score_multiplier_func_2W(player["word"], player["x"], player["y"], player["direction"], arr) != False:
        ...
        if len(player["joker_letter"]) != 0:
            score -= 2*sum(player["joker_value"])
            player["joker_value"] = []     
    if score_multiplier_func_3L(player, player["word"], player["x"], player["y"], player["direction"], letter_scores, arr) != False:
        score += score_multiplier_func_3L(player, player["word"], player["x"], player["y"], player["direction"], letter_scores, arr)
    if score_multiplier_func_2L(player, player["word"], player["x"], player["y"], player["direction"], letter_scores, arr) != False:
        score += score_multiplier_func_2L(player, player["word"], player["x"], player["y"], player["direction"], letter_scores, arr)
    if len(player["joker_value"]) != 0:
        for i in player["joker_value"]:
            score -= i    
    player["score"] += score
def printing_word(player, inp, x, y, direction, joker_replace, arr, joker_status, joker_coords):
    p = 0
    joker_pos = 0
    for i in range(len(inp)):
        if direction == "R":
            if arr[int(y)-1][int(x)-1+p] != inp[p]:
                ...
                player["letters"].remove(player["input"][i])
...
def print_score(player, score, player_switch):
    player["score_memory"].append(score)
    print("Player", player_switch)
    for i in range(len(player["score_memory"])):                                       
...
def load_whole_word(player, x, y, direction, arr):
    new_word = ""
    p = 0
    p_main = 1
    move = 0
        ...
        for i in player["input"]:
            ...
            if len(new_word) <= 1:
                ...
                if valid_english_word(new_word) == False:
                    ...
                    move += 1
def add_before_after(player, word, x, y, direction, arr):
    new_word = word
    p = 1
    if direction == "D":
    ...
    if direction == "R":
        ...
        p = 1
        player["word"] = new_word
def locate_joker(player, joker_replace, letter_scores):
    joker_value = letter_scores[joker_replace]
    player["joker_value"].append(joker_value)
    player["joker_letter"].append(joker_replace)                                           
    for i in range(len(player["letters"])):
        if player["letters"][i] == "joker":
            player["letters"][i] = joker_replace
def joker_pickup_engine(player, joker_picked_position, player_switch, arr, joker_y, joker_x, joker_coords):    
    if joker_picked_position in joker_coords:
        player["letters"].append("joker")
        player["letters"].remove(arr[int(joker_y)-1][int(joker_x)-1])
        print("Player",player_switch, player["letters"])
...
def valid_english_word(word):
    if word in en_words:
        ...
        return False
def letters_inhand_checker(player, inp, x, y, direction, arr):
    p = 0
    for i in inp:
        if direction == "D":
            if i in player["letters"] or i == arr[int(y)-1+p][int(x)-1]:
                return True
            else:
                print("Error, you don't have the letter", i)
                return False
        if direction == "R":
            if i in player["letters"] or i == arr[int(y)-1][int(x)-1+p]:
                return True
            else:
                print("Error, you don't have the letter", i)
                return False
...
def score_multiplier_func_2W(word, x, y, direction, arr):
    ...
    if counter > 0:
        ...
        return False
def score_multiplier_func_3L(player, word, x, y, direction, letter_scores, arr):
    p = 0
    counter = 0
    for i in word:
        if direction == "R":
            if arr[int(y)-1][int(x)-1+p] == 2:
                ...
                if letter_scores[i] in player["joker_value"]:
                    counter -= 2*letter_scores[i]
                    player["joker_value"].remove(letter_scores[i])
            else:
                p += 1
        elif direction == "D":
    ...
    if counter > 0:
        ...
        return False
def score_multiplier_func_2L(player, word, x, y, direction, letter_scores, arr):
    p = 0
    counter = 0
    for i in word:
        if direction == "R":
